...
2. **practices/ catalog** — Java/Spring best-practices 221룰 + practices-react 99룰 + ESLint 14룰. evidence-anchored (외부 URL/quote 필수)라 AI가 임의로 룰을 발명하지 못함.
...
- **언어/프레임워크 확장** — Java/Spring 카탈로그(221 rules) + React/Next.js 카탈로그(99 rules + 14 ESLint rules) 둘 다 active. 다른 stack(Kotlin/Go/Python 등) 추가는 동일 패턴 (spec → rule → evidence → test) 따라 확장.
...
| `./gradlew testPractices`      | GREEN    | 221 rules PASS |
...
cd backend && ./gradlew testPractices # practices/ 221룰 검증 (GREEN)
...
| Practices | `specs/spring-practices-l0.yaml` | — | 221 rules / 22 categories |
