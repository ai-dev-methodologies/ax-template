...
```
fork ax-template
    ↓ (25 L4 domains + 11 active recipes + 233 Java rules + 102 React rules + 15 ESLint rules + 100 hard guards + AGENTS.md sentinel)
...
│   ├── evals/                 # 4 hard gates + 100 hard guards
...
| `run-all-guards.sh` (94 live guards) | `practices/evals/run-all-guards.sh` | R25 완료 선언 시 수동 호출 (verify-completion.sh 내부에서 실행) | **manual / R25 run** — 자동 트리거 없음 | 항상 사용 가능, 자동 실행 아님 |
