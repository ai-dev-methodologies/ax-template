...
```
fork ax-template
    ↓ (25 L4 domains + 11 active recipes + 233 Java rules + 102 React rules + 15 ESLint rules + 109 hard guards + AGENTS.md sentinel)
...
- **언어/프레임워크 확장** — Java/Spring 카탈로그(233 rules) + React/Next.js 카탈로그(102 rules + 15 ESLint rules) 둘 다 active. 다른 stack(Kotlin/Go/Python 등) 추가는 동일 패턴 (spec → rule → evidence → test) 따라 확장.
