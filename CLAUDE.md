...
```
fork ax-template
    ↓ (25 L4 domains + 11 active recipes + 186 Java rules + 99 React rules + 8 ESLint rules + 73 hard guards + AGENTS.md sentinel)
...
2. **practices/ catalog** — Java/Spring best-practices 186룰 + practices-react 86룰 + ESLint 7룰. evidence-anchored (외부 URL/quote 필수)라 AI가 임의로 룰을 발명하지 못함.
...
- **언어/프레임워크 확장** — Java/Spring 카탈로그(186 rules) + React/Next.js 카탈로그(99 rules + 8 ESLint rules) 둘 다 active. 다른 stack(Kotlin/Go/Python 등) 추가는 동일 패턴 (spec → rule → evidence → test) 따라 확장.
...
- `./gradlew testPractices` — 186 practices rules
...
| `./gradlew testPractices`      | GREEN    | 186 rules PASS |
...
cd backend && ./gradlew testPractices # practices/ 186룰 검증 (GREEN)
...
│   ├── rules/                 # 186룰, 22+ categories (R50/R58/R61 추가분 포함)
...
| Practices | `specs/spring-practices-l0.yaml` | — | 186 rules / 22 categories |
