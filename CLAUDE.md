...
```
fork ax-template
    ↓ (25 L4 domains + 11 active recipes + 223 Java rules + 99 React rules + 14 ESLint rules + 84 hard guards + AGENTS.md sentinel)
...
| `run-all-guards.sh` (84 guards, 144 invocations) | `practices/evals/run-all-guards.sh` | R25 완료 선언 시 수동 호출 (verify-completion.sh 내부에서 실행) | **manual / R25 run** — 자동 트리거 없음 | 항상 사용 가능, 자동 실행 아님 |
