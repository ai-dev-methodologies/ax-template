...
| `verify-downstream.sh` (downstream-fixture E2E) | `practices/scripts/verify-downstream.sh` | 사람이 호출 — stock-shaped 2-스택 소비자 픽스처에 설치 산출물을 **SKILL.md 마커에서 추출·verbatim 설치**하고 "위반이 실제로 차단되는가"를 **26개 단언**으로 측정(2026-08-18 재계산 — `# ax:assertions` 선언 실측; 13개이던 시점은 P2-103~P2-106 웨이브가 husky/lefthook 분기·worktree·대체 태스크명·`react.root: "."`용 8개를 신설하며 23개로 늘었다: `A11-route`·`A12-route`·`A13-husky`·`A14-lefthook`·`A15-worktree`·`A16-alttask`·`A17-alttask`·`A18-rootdot`·`A19-rootdot`·`A20-rootdot-skip` 중 신규 10개가 원 13에 얹혀 23. **같은 날 등록·원장 레인이 P2-121~123을 봉합하며 3개를 더 신설해 23 → 26**: `A21-plugindep`(react 플러그인이 `<react.root>`에 설치·선언되는가, base=repo 사고 배제) · `A15b-preflight`(F-2 worktree preflight 조건을 detector artifact로 승격해 양방향 실행) · `A13b-posix`(husky shim의 shebang 유실 — dash 계열 `/bin/sh`에서 `set -euo pipefail`이 죽는 것의 정적+동적 검사)) — **차단 단언은 exit code + 게이트 자신의 신호 문자열을 함께 요구**하고, `A6`·`A7b`는 그 차단 단언이 공허해지지 않도록 전제를 재는 **premise 단언**이다(전자와 성격이 다르므로 구분해 적는다) | **periodic / manual** — merge gate 아님. 단 그 **로그가 [114]의 입력**이므로 릴리스에는 사실상 필수 | 항상 사용 가능(네트워크 + JDK 21 + node 필요; 전제 미달 시 "PREMISE NOT ESTABLISHED"로 **무측정** 명시) |
...
| **verify-downstream.sh** (설치 산출물) | **살아있는 결함으로 falsification-proven** — 수정 전(`a2551141`) 산출물을 `--artifact-override`로 주입해 결함별 RED를 실측: GH #89→**A0** RED(`method that in class JavaClasses cannot be applied to given types`) · GH #90→**A7** RED(`Could not create task ':testPractices'`) · GH #91→**A8** RED(`practicesGateActuallyScansTheProject() FAILED`) · F-030→**A1+A2** RED(`ENOENT ./ax.config.json`) · F-034→**A-pc+A5+A6** RED(배너 부재). 무주입 대조군은 11/11 PASS. 추가로 #78/#79/#86 형상 주입 시 각각 A4/A2/A3 RED. *(이 표본은 단언이 11개이던 시점의 것이다 — 이후 `A9-eval`·`A10-tsdep`가 추가돼 명부는 13개이며, [114]가 그 명부를 pushed sha에서 도출해 정확일치를 요구한다.)* *(2026-08-18 재계산: 명부는 이후 P2-103~P2-106 웨이브가 `A11-route`~`A20-rootdot-skip` 10개를 더 신설해 23개가 됐다 — 위 살아있는-결함 표본 자체는 갱신되지 않았으며 여전히 그때의 11/13 세계를 서술한다.)* *(2026-08-18 등록·원장 레인 재계산: 같은 날 안에서 P2-121~123 봉합이 `A21-plugindep`·`A15b-preflight`·`A13b-posix` 3개를 더 신설해 명부는 **23 → 26**개다 — 위 살아있는-결함 표본은 여전히 갱신되지 않았고, 신규 3건 자신의 pre-fix→post-fix 차등은 이 표가 아니라 BACKLOG P2-121~123 행에 있다.)* |
