...
| `./gradlew testFavorites`      | GREEN    | 12/12 PASS. R34 backend impl: Favorite entity (polymorphic via entity_type/entity_id pair + UNIQUE(user_id, entity_type, entity_id)) + FavoriteService (idempotent add/delete, global count, quota enforcement) + FavoriteController (5 endpoints). 12 spec items / 4 families (CRUD × 3, QUERY × 3, AUTHZ × 3, VALIDATION × 3). Single-state toggle pattern (like / save / bookmark / star). HTTP DELETE idempotent per RFC 9110 §9.3.5 — second DELETE on absent row returns 204. Caller derived from Authentication only (no userId path/query). Explicit MethodArgumentNotValidException → 400 handler pins validation status (otherwise upstream filter chain re-classifies to 403). |
