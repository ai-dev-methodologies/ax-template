...
```
fork ax-template
    ↓ (23 L4 domains + 11 active recipes + 112 Java rules + 86 React rules + 7 ESLint rules + 64 hard guards + AGENTS.md sentinel)
...
2. **practices/ catalog** — Java/Spring best-practices 112룰 + practices-react 86룰 + ESLint 7룰. evidence-anchored (외부 URL/quote 필수)라 AI가 임의로 룰을 발명하지 못함.
...
- **언어/프레임워크 확장** — Java/Spring 카탈로그(112 rules) + React/Next.js 카탈로그(86 rules + 7 ESLint rules) 둘 다 active. 다른 stack(Kotlin/Go/Python 등) 추가는 동일 패턴 (spec → rule → evidence → test) 따라 확장.
...
- `./gradlew testPractices` — 112 practices rules
...
| `./gradlew testPractices`      | GREEN    | 112 rules PASS |
...
cd backend && ./gradlew testPractices # practices/ 112룰 검증 (GREEN)
...
│   ├── evals/                 # 4 hard gates + 64 hard guards
...
│   └── L4/                    # 23 domain verticals (full-trio 20 + INFRA 1 + backend-only 2: multi-tenant, i18n-policy)
...
├── docs/
│   ├── IMPLEMENTATION-STATUS.md  # 23 L4 status taxonomy (refreshed 2026-05-30)
...
| Practices | `specs/spring-practices-l0.yaml` | — | 112 rules / 22 categories |
