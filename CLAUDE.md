...
```
fork ax-template
    ↓ (25 L4 domains + 11 active recipes + 117 Java rules + 86 React rules + 7 ESLint rules + 70 hard guards + AGENTS.md sentinel)
...
2. **practices/ catalog** — Java/Spring best-practices 117룰 + practices-react 86룰 + ESLint 7룰. evidence-anchored (외부 URL/quote 필수)라 AI가 임의로 룰을 발명하지 못함.
...
- **언어/프레임워크 확장** — Java/Spring 카탈로그(117 rules) + React/Next.js 카탈로그(86 rules + 7 ESLint rules) 둘 다 active. 다른 stack(Kotlin/Go/Python 등) 추가는 동일 패턴 (spec → rule → evidence → test) 따라 확장.
...
- `./gradlew testPractices` — 117 practices rules
...
| `./gradlew testPractices`      | GREEN    | 117 rules PASS |
...
cd backend && ./gradlew testPractices # practices/ 117룰 검증 (GREEN)
...
│   ├── rules/                 # 117룰, 22+ categories (R50/R58/R61 추가분 포함)
...
| Practices | `specs/spring-practices-l0.yaml` | — | 117 rules / 22 categories |
