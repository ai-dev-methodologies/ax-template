...
| `./gradlew testApprovalWorkflow` | GREEN  | 19/19 PASS. R31 backend impl: ApprovalRequest + ApprovalStep entities (OneToMany cascade) + ApprovalRequestStatus(DRAFT/SUBMITTED/APPROVED/REJECTED/CANCELLED) + ApprovalStepStatus(PENDING/APPROVED/REJECTED) + ApprovalRequestStateMachine + ApprovalStepStateMachine (both sole-mutator) + ApprovalService (single @Transactional cascades — last step approve ⇒ request APPROVED; any step reject ⇒ request REJECTED). 12 spec items / 4 families (LIFECYCLE × 4, AUTHZ × 3, STEP × 3, QUERY × 2). 한국 enterprise 결재 패턴 (sequential ordered, 결재선). Inbox query uses repository JPQL JOIN on PENDING steps of SUBMITTED requests. |
