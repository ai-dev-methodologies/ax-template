...
| `./gradlew testTagCategorization` | GREEN | 19/19 PASS. R32 backend impl: Tag + TagAttachment entities (polymorphic via entity_type/entity_id pair) + TagSlugger (NFKD normalize + ASCII filter + hyphenate + Korean fallback) + TagService (idempotent attach/detach, cascade delete with subtree recursion) + TagController (@PreAuthorize ROLE_ADMIN on definition mutations). 12 spec items / 4 families (CRUD × 4, ATTACHMENT × 3, HIERARCHY × 3, AUTHZ × 2). FK ON DELETE CASCADE on tag_attachments + UNIQUE (tag_id, entity_type, entity_id) backs idempotent attach. parentTagId @Column(updatable=false) = cycle defense. |
