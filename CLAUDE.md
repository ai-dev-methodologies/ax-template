...
```
fork ax-template
    ↓ (25 L4 domains + 11 active recipes + 233 Java rules + 102 React rules + 15 ESLint rules + 97 hard guards + AGENTS.md sentinel)
...
│   ├── evals/                 # 4 hard gates + 97 hard guards
...
| `run-all-guards.sh` (97 guards) | `practices/evals/run-all-guards.sh` | R25 완료 선언 시 수동 호출 (verify-completion.sh 내부에서 실행) | **manual / R25 run** — 자동 트리거 없음 | 항상 사용 가능, 자동 실행 아님 |
...
- **97개 guard는 `run-all-guards.sh` 경유 수동 실행 전용이다** (practices/evals 95 + practices-react/evals 2; 여기에 pre-push 전용 recency guard 1을 더해 총 98 hard guards). 커밋마다 자동 실행되지 않는다.
