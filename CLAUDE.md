...
```
fork ax-template
    ↓ (25 L4 domains + 11 active recipes + 233 Java rules + 102 React rules + 15 ESLint rules + 94 hard guards + AGENTS.md sentinel)
...
- **94개 guard는 `run-all-guards.sh` 경유 수동 실행 전용이다** (practices/evals 92 + practices-react/evals 2; 여기에 pre-push 전용 recency guard 1을 더해 총 95 hard guards). 커밋마다 자동 실행되지 않는다.
