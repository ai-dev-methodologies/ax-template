...
if [ -n "${AX_GIT_BIN:-}" ] \
   && "$AX_GIT_BIN" --no-replace-objects -C "$SELF_REPO_ROOT" rev-parse --git-dir >/dev/null 2>&1; then
    ...
    if [ -n "$_ax_pf_want" ]; then
        _ax_pf_priv="$(_ax_mktemp_d "evidence_quote_spotcheck_guard")" || exit 2
        if ! cat -- "$AX_ANCHOR_LIB" > "$_ax_pf_priv/release_anchor.sh" 2>/dev/null; then
            echo "evidence_quote_spotcheck_guard: HELPER_BYTES_UNBOUND — practices/scripts/lib/release_anchor.sh could not be copied into a" >&2
        ...
        if [ "$_ax_pf_want" != "$_ax_pf_have" ]; then
            echo "evidence_quote_spotcheck_guard: HELPER_BYTES_UNBOUND — the copy of practices/scripts/lib/release_anchor.sh this run is" >&2
