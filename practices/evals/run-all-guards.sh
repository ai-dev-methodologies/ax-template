...
echo "[116] test_support_response_validation_guard.sh (BACKLOG P2-117 — a backend/src/test/**/*TestSupport.java helper MUST NOT read a value out of an HTTP response it has not described. P2-116 measured the four ways a blind \`.then().extract().path(\"field\")\` fails against this tree's exact rest-assured: no Content-Type header at all produces an IllegalStateException naming nothing (the P3-144 preimage — the response's ONLY preimage for that exact message is 'this app never answered'); 200 with an empty body produces a parse error naming nothing; 401 with a problem+json body produces NO exception at all, just a null silently carried forward as if it were the value; port 0 throws before any response exists. P2-117's census found this exact blind-read shape in 86 of the 221 files that call \`extract()\` — precisely the *TestSupport.java files, none of which carried a status assertion anywhere in the chain (the other 135 already do, so widening the scan there would flag files that are already described). All 86 were moved onto the new \`com.ax.template.authblueprint.common.HttpExtract\` helper, which asserts status, then Content-Type presence, then the JSON family, then the extraction, then the value's presence — and on failure reports status / content-type / every header / a bounded body excerpt / RestAssured.port. Moving 86 files does not stop the 87th, since a new domain's TestSupport is normally written by copying a neighbour — so the guard enforces the recipe structurally: (A) an \`.extract()\` whose next chained call is anything but \`.response()\` (the sanctioned handoff, and a terminal \`.extract()\` that hands the whole ExtractableResponse onward is equally sanctioned) is BLOCKED; (B) a \`.path(\`/\`.pathAt(\`/\`.jsonPath(\` call whose qualifier is not \`HttpExtract\` is BLOCKED, closing the two-statement evasion \`var r = ...extract(); r.path(\"x\");\` that no regex on (A) alone would ever see. NO ALLOWLIST — the target set is derived from disk on every run via \`find ... -name '*TestSupport.java'\`, so a new domain is inside the rule the moment its file lands; an exception list is how this exact class of guard goes quietly vacuous (the file that most needs the rule is the file most likely to be excused from it). \`HttpExtract.java\` itself calls \`response.path(...)\` and is not scanned — excluded by construction (its name does not end in TestSupport.java), not by a name on a list. DELIBERATELY NOT DONE: general test classes outside *TestSupport.java are out of scope (the 135 already-described files this would flag buy nothing); no parsing of whether a chain's \`statusCode(...)\` exists elsewhere (a fragile multi-line parse with no single answer); no check that a helper's \`context\` string argument is actually informative (a review judgement, not a binary). Exit: 0 PASS · 1 an unvalidated response read found · 2 usage/environment error. Fixtures: pass_via_helper 0 (a TestSupport reading exclusively through HttpExtract) / fail_blind_extract 1 (reproduces all three of: a bare \`.extract().path(...)\`, a two-statement \`var r = ...extract(); r.path(...)\` evasion of check (A), and a bare \`.extract().asString()\`).)"
