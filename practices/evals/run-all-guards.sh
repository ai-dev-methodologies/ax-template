...
if [ "$INCLUDE_FIXTURES" -eq 1 ]; then
    run_guard "checklist_command_path_spelling/fixtures" 0 \
...
# ── 107. cross_catalog_upstream_id_collision_guard ──────────────────────────
echo "[107] cross_catalog_upstream_id_collision_guard.sh (BACKLOG P3-137 — ONE upstream_id registered in BOTH catalogs against TWO DIFFERENT sources. evidence_quote_spotcheck's resolve_snapshot_any_catalog tries practices/upstream FIRST and practices-react second, so a templates/** citation names an ID, not a catalog: when the two catalogs hold different pages under one id, whether a quoted sentence is TRUE is decided by that loop and not by the citing file. MEASURED, not theorised (Lane G): with wcag-22-techniques-2026-05 holding the SC 4.1.3 UNDERSTANDING page on the practices side and the Techniques INDEX on the react side, renaming the practices-side copy away in a throwaway clone flipped the protected template sweep from exit 0 to exit 1 with 4 findings — same citations, same bytes, opposite verdict. THE INVARIANT IS AGREEMENT, NOT EXCLUSIVITY: live census at introduction is 61 practices ids x 42 practices-react ids sharing exactly 4 — next-themes / stripe-billing / toss-billing carry an IDENTICAL source on both sides (one upstream fact legitimately cited twice) and only wcag-22-techniques names two pages, so forbidding shared ids outright would block three honest ones to catch one dishonest one. THE ONE LIVE COLLISION IS GRANDFATHERED BECAUSE IT PROVABLY CANNOT BE DISSOLVED, measured first-hand in throwaway clones of cf42258: renaming the REACT side (git mv + manifest id) exits 2 RECEIPT_ORPHANED in manifest_snapshot_integrity_guard; renaming the PRACTICES side exits 2 RECEIPT_ORPHANED there AND exit 1 / 66 file(s) / 70 anchor(s) / 4 finding(s) in the protected spotcheck, because the two pinned template anchors stop resolving; and Lane G separately measured that a count-preserving RESPELL of a pinned identity is exit 5 PROTECTED_IDENTITY_REMOVED ('the pin set may only GROW'). _FETCH-RECEIPTS.yaml is append-only and binds an id to the body a fetch produced, so an id is part of a provenance record rather than a label that can be re-spelled — relabelling would be the 'manufacture a provenance claim' move the receipts chain exists to prevent. The exception is a FROZEN frozenset literal in the guard (growing it is a reviewed edit to a guard, not a line appended to a data file) and it CANNOT become padding: NON-REDUNDANCY fails a grandfathered id that is registered in both catalogs and now agrees (any root), and additionally one that is not shared at all any more (live root only — a fixture root has no reason to carry this repo's ids, and requiring it would make the exception's own test untestable). Also blocks SHARED_SNAPSHOT_UNREGISTERED: a body present in both upstream dirs while one manifest never registered it, because resolution is by FILE and 'the sources cannot be compared at all' is worse than 'they disagree'. Non-vacuity: LIVE_MIN_SHARED=4 floor on a live root (exit 2) so a collapsed census cannot report a green 'no collisions' about an empty set. Fixtures: pass_shared_ids_agree 0 / fail_same_id_two_sources 1 / fail_stale_grandfather 1 / fail_shared_body_unregistered 1; the collision fixture is [87] kill-proof registered. Live exits 0 at 4 shared ids.)"
run_guard "cross_catalog_upstream_id_collision/live" 0 \
    bash "$SCRIPT_DIR/cross_catalog_upstream_id_collision_guard.sh"
if [ "$INCLUDE_FIXTURES" -eq 1 ]; then
    run_guard "cross_catalog_upstream_id_collision/fixture_pass_shared_ids_agree" 0 \
        bash "$SCRIPT_DIR/cross_catalog_upstream_id_collision_guard.sh" --root "$SCRIPT_DIR/fixtures/cross-catalog-upstream-id/pass_shared_ids_agree"
    run_guard "cross_catalog_upstream_id_collision/fixture_fail_same_id_two_sources" 1 \
        bash "$SCRIPT_DIR/cross_catalog_upstream_id_collision_guard.sh" --root "$SCRIPT_DIR/fixtures/cross-catalog-upstream-id/fail_same_id_two_sources"
    run_guard "cross_catalog_upstream_id_collision/fixture_fail_stale_grandfather" 1 \
        bash "$SCRIPT_DIR/cross_catalog_upstream_id_collision_guard.sh" --root "$SCRIPT_DIR/fixtures/cross-catalog-upstream-id/fail_stale_grandfather"
    run_guard "cross_catalog_upstream_id_collision/fixture_fail_shared_body_unregistered" 1 \
...
echo "[112] install_artifact_extractability_guard.sh (GH #92 step-in-progress — the downstream verification harness (practices/scripts/verify-downstream.sh) EXTRACTS what it installs into a consumer project from the <!-- ax:artifact ... --> markers embedded in skills/ax-install-{hooks,java-enforcement,react-enforcement}/SKILL.md, and a malformed marker does not fail loudly there: the harness either extracts nothing for that artifact or extracts something silently wrong, and reports whatever it DID manage to install as a green run. This guard closes that silence at the SOURCE, before the harness ever runs. Checks: (1-8) delegated verbatim to practices/scripts/lib/ax_markers.py's discover()+lint() — the SAME parser verify-downstream.sh itself consumes, so an extractor bug is a bug in ONE place rather than two independently-maintained sweeps silently diverging — covering marker<->fence 1:1, id uniqueness, fence-language registration, ax:if/ax:endif balance, directive-prefix-matches-fence-language, substs<->body/path bidirectional agreement, free-text conditional prose surviving outside a directive line, required/unknown/duplicate attribute names, kind=file-fragment requiring merge=, and (P2-109) every config.* reference in when=/ax:if/ax:subst cross-checked against the committed ax.config.schema.json (UNKNOWN_CONFIG_PATH); (9) COVERAGE, guard-owned — every skills/ax-install-*/SKILL.md (globbed from disk, never a hardcoded list) must carry >=1 marker; (9b) ZERO-CENSUS BLOCK (P2-109) — a scanned root with no skills/ax-install-* directory at all is NO_INSTALL_SKILLS, not a silent exit 0; (10) SHELL RENDERABILITY, guard-owned — every bash/sh-fenced artifact is rendered via ax_markers.render() against the real consumer-e2e fixture's ax.config.json and then run through \`bash -n\`, since a marker can be structurally well-formed by every one of lint()'s rules and still render invalid shell. Exit: 0 PASS · 1 violation(s) found (checks 1-10, incl. 9b) · 2 usage/setup error. Fixtures: pass_marked 0 / fail_missing_marker 1 (delegated, MARKER_NO_FENCE) / fail_duplicate_id 1 (delegated, DUPLICATE_ID) / fail_free_text_conditional 1 (delegated, FREE_TEXT_CONDITIONAL) / fail_wrong_comment_prefix 1 (delegated, DIRECTIVE_PREFIX_MISMATCH) / fail_missing_required_attr 1 (delegated, MISSING_REQUIRED_ATTR) / fail_unknown_attr 1 (delegated, UNKNOWN_ATTR) / fail_unknown_config_path 1 (delegated, UNKNOWN_CONFIG_PATH) / fail_missing_merge_for_fragment 1 (delegated, MISSING_MERGE_FOR_FRAGMENT) / fail_zero_install_skills 1 (check 9b, NO_INSTALL_SKILLS) / fail_unparseable_shell 1 (check 10, UNPARSEABLE_SHELL).)"
...
if [ "$INCLUDE_FIXTURES" -eq 1 ]; then
    ...
    run_guard "install_artifact_extractability/fixture_fail_missing_required_attr" 1 \
        bash "$SCRIPT_DIR/install_artifact_extractability_guard.sh" --root "$SCRIPT_DIR/fixtures/install_artifact_extractability/fail_missing_required_attr"
    run_guard "install_artifact_extractability/fixture_fail_unknown_attr" 1 \
        bash "$SCRIPT_DIR/install_artifact_extractability_guard.sh" --root "$SCRIPT_DIR/fixtures/install_artifact_extractability/fail_unknown_attr"
    run_guard "install_artifact_extractability/fixture_fail_unknown_config_path" 1 \
        bash "$SCRIPT_DIR/install_artifact_extractability_guard.sh" --root "$SCRIPT_DIR/fixtures/install_artifact_extractability/fail_unknown_config_path"
    run_guard "install_artifact_extractability/fixture_fail_missing_merge_for_fragment" 1 \
        bash "$SCRIPT_DIR/install_artifact_extractability_guard.sh" --root "$SCRIPT_DIR/fixtures/install_artifact_extractability/fail_missing_merge_for_fragment"
    run_guard "install_artifact_extractability/fixture_fail_zero_install_skills" 1 \
        bash "$SCRIPT_DIR/install_artifact_extractability_guard.sh" --root "$SCRIPT_DIR/fixtures/install_artifact_extractability/fail_zero_install_skills"
...
if [ "$INCLUDE_FIXTURES" -eq 1 ]; then
    ...
    run_guard "cross_artifact_contract/fixture_fail_prose_only_p_contract" 1 \
        bash "$SCRIPT_DIR/cross_artifact_contract_guard.sh" --root "$SCRIPT_DIR/fixtures/cross_artifact_contract/fail_prose_only_p_contract"
...
echo "[116] test_support_response_validation_guard.sh (BACKLOG P2-117 + P2-119 — nothing under \`backend/src/test/**\` may read a value out of an HTTP response whose status nobody described. P2-116 measured the four ways a blind \`.then().extract().path(\"field\")\` fails against this tree's exact rest-assured: no Content-Type header at all produces an IllegalStateException naming nothing (the P3-144 preimage — that exact message's ONLY preimage is 'this app never answered'); 200 with an empty body produces a parse error naming nothing; 401 with a problem+json body produces NO exception at all, just a null silently carried forward as if it were the value; port 0 throws before any response exists. TWO LAYERS, because the two layers have different information available. LAYER 1 — \`*TestSupport.java\` (P2-117): a shared helper has no local place to describe a response (whether 201 or 409 is correct depends on the caller), so the rule there is absolute — every value read goes through \`com.ax.template.authblueprint.common.HttpExtract\`, which asserts status, then Content-Type presence, then the JSON family, then the extraction, then the value's presence, and on failure reports status / content-type / every header / a bounded body excerpt / RestAssured.port. Detectors (A) an \`.extract()\` whose next chained call is neither \`.response()\` (the sanctioned handoff) nor \`.statusCode()\` (which reads the STATUS — never null, cannot fail to parse, and is how a test DESCRIBES a response rather than a way of failing to), and (B) a \`.path(\`/\`.pathAt(\`/\`.jsonPath(\` whose qualifier is not \`HttpExtract\`, which closes the two-statement evasion \`var r = ...extract(); r.path(\"x\");\` that no regex on (A) alone could see. LAYER 2 — every other test class (P2-119): a test METHOD is exactly the place that knows which status is correct, and this tree already says so 1404 times, so the rule there is that the response must be DESCRIBED. Detector (C) a \`*TestSupport.java\` method whose DECLARED RETURN TYPE is a rest-assured response type (Response / ExtractableResponse / ValidatableResponse / ResponseBody / JsonPath / ResponseOptions) is BLOCKED — this is P2-119's named residue, invisible to (A)/(B) because handing the whole response out is legal under (A) precisely because the CALLER might validate it, and when the caller does not, the blind read lands in a file (A)/(B) never scan; and detector (D) a value read (the same shapes as (A)/(B)) in a non-TestSupport test class whose response object is not described — by \`statusCode(\`/\`spec(\` in the read's own chain, or by a chain ROOTED AT the receiving local carrying \`statusCode(\`/\`getStatusCode(\`, or by the local's assignment, or by a SAME-FILE helper method whose body pins a status, or by \`HttpExtract\` — is BLOCKED. (C) IS (D)'S SOUNDNESS PREMISE, not an independent nicety: P2-117 recorded 'no status-assertion parsing' as deliberately not done because an assertion may live in a DIFFERENT FILE from the extraction, so at the TestSupport layer the question has no local answer; once no TestSupport may export a response, a response object inside a non-TestSupport class can only have come from a chain built in that same file, and 'was this described' becomes an intra-file question with one answer. The narrower objections are handled mechanically rather than avoided — chains that span lines are LEXED not grepped (comments, string/char literal contents and Java text blocks are blanked with offsets preserved, so a javadoc quoting the forbidden shape and a diagnostic string carrying it verbatim are both invisible), and chains built in fragments are followed through the local name and through same-file helpers, with overloads AND-folded so a described 3-arg overload cannot launder an undescribed 2-arg delegate. P2-119 CENSUS (re-measured against bd19e251 with these exact rules; the definition is stated because the definition DOMINATES the result): over 412 non-TestSupport test files, the narrow literal chain \`.extract().path(\` occurs at 209 sites / 91 files, while the full (A)+(B) literal shape set reaches 1710 sites / 135 files — an 8x spread on definition alone, which is why the seed census recorded both. Counting the guard's own MODELLED read sites (every (A)/(B) site, sanctioned followers excluded, no same-line dedup) gives 1984 sites / 134 files, and ORIGIN RESOLUTION splits them 1651 described / 333 undescribed — the deduped violation report names 277 of those, plus the 1 detector-(C) export, for 278. So the blind population is ~17% of the shape matches: moving the described 1651 onto the helper would have been pure diff with no diagnosability gained, which is exactly why (D) resolves the origin instead of matching the line. All 278 were sealed (after: 1982 modelled sites, 1982 described, 0 undescribed) and the axis is now enforced. NO ALLOWLIST — the target sets are derived from disk on every run (a walk of backend/src/test), no count is hardcoded, and a new domain is inside the rule the moment its file lands; an exception list is how this exact class of guard goes quietly vacuous, since the file that most needs the rule is the file most likely to be excused from it. \`HttpExtract.java\` itself calls \`response.path(...)\` and is not scanned — the SUBJECT/OBJECT distinction, derived from the file's own identity as the validated extraction, not a name on a list. The one place an exception was TEMPTING is recorded because it was resolved without one: HttpExtractDiagnosabilityTest deliberately performs blind reads to DEMONSTRATE that they fail silently, and pinning the status those demonstrations already know (\`r.then().statusCode(401).extract().path(\"accessToken\")\` still yields null) keeps them verbatim and makes them strictly stronger — they now show that a status assertion ALONE is insufficient, which is why HttpExtract also asserts the value's presence. DELIBERATELY NOT DONE: inter-procedural PARAMETER flow (a read whose receiver is a method parameter is treated as described — whether it was is a property of the call sites, and the guard declines to chase them; 5 such sites exist on this tree); cross-file helper resolution for non-TestSupport files (reported as undescribed, i.e. fail-closed, so a false-positive risk rather than a hole — and no such shape exists now that (C) removes the only class of file that exported responses); whether the asserted status is the RIGHT status (\`statusCode(500)\` describes a response as surely as \`statusCode(200)\`; which one a test should expect is the test's claim, not this guard's); whether an \`HttpExtract\` \`context\` argument is informative (a review judgement, not a binary); and reads that never touch path/pathAt/jsonPath and never follow \`.extract()\` (no silent-null failure mode — the P2-116 measurement is specifically about GPath extraction). Exit: 0 PASS / 1 an unvalidated response read or an undescribed response export / 2 usage-environment error. Fixtures: pass_via_helper 0 (a TestSupport reading exclusively through HttpExtract and exporting nothing, plus a caller test exercising EVERY description form — in-chain, on-the-local, via-then, same-file-helper, HttpExtract, status-read, parameter — so the guard is proven not to fire on correct code) / fail_blind_extract 1 (all three of a bare \`.extract().path(...)\`, the two-statement \`var r = ...extract(); r.path(...)\` evasion of (A), and a bare \`.extract().asString()\`) / fail_testsupport_exports_response 1 (detector (C) IN ISOLATION — every \`.extract()\` is terminal, no reader call appears anywhere, and there is no non-TestSupport class, so (A)/(B)/(D) cannot fire and MUT-C alone flips it 1->0) / fail_caller_blind_read 1 (detector (D) IN ISOLATION — no \`*TestSupport.java\` exists in the tree at all, so (A)/(B)/(C) cannot fire and MUT-D alone flips it 1->0; three origins are represented — inline chain, undescribed local, undescribed same-file helper — so a partial resolver regression is visible in the violation COUNT).)"
run_guard "test_support_response_validation/live" 0 \
    bash "$SCRIPT_DIR/test_support_response_validation_guard.sh"
if [ "$INCLUDE_FIXTURES" -eq 1 ]; then
    run_guard "test_support_response_validation/fixture_pass_via_helper" 0 \
        bash "$SCRIPT_DIR/test_support_response_validation_guard.sh" --root "$SCRIPT_DIR/fixtures/test_support_response_validation/pass_via_helper"
    run_guard "test_support_response_validation/fixture_fail_blind_extract" 1 \
        bash "$SCRIPT_DIR/test_support_response_validation_guard.sh" --root "$SCRIPT_DIR/fixtures/test_support_response_validation/fail_blind_extract"
    run_guard "test_support_response_validation/fixture_fail_testsupport_exports_response" 1 \
        bash "$SCRIPT_DIR/test_support_response_validation_guard.sh" --root "$SCRIPT_DIR/fixtures/test_support_response_validation/fail_testsupport_exports_response"
    run_guard "test_support_response_validation/fixture_fail_caller_blind_read" 1 \
        bash "$SCRIPT_DIR/test_support_response_validation_guard.sh" --root "$SCRIPT_DIR/fixtures/test_support_response_validation/fail_caller_blind_read"
fi

echo "[117] restassured_port_single_writer_guard.sh (BACKLOG P2-120 — nothing under \`backend/src/test/**\` may ASSIGN the process-global \`io.restassured.RestAssured.port\`; it has exactly ONE writer, the JUnit extension named in \`backend/src/test/resources/META-INF/services/org.junit.jupiter.api.extension.Extension\`, which reads each test instance's @LocalServerPort immediately before the test and RECORDS what it published. Reading the field stays legal; only writing is refused. TWO detector shapes, both kill-proven: (A) a qualified write \`RestAssured.port = ...\`, and (B) a static import of that member — BOTH the single-name form \`...RestAssured.port;\` AND the wildcard form \`...RestAssured.*;\`, which brings \`port\` in under its simple name so a bare \`port = ...\` is the same write. The wildcard arm was added 2026-08-17 after a fable5 adversarial review REPRODUCED it as a false negative (guard exited 0); since this tree's dominant idiom is \`import static io.restassured.RestAssured.given;\`, changing \`given\` to \`*\` was a one-character bypass sitting inside the guard's own stated threat model. The scanner also lexes Java text blocks (\`\"\"\"\`) so a forbidden line quoted inside one is not scanned as code — the same review reproduced that as a false POSITIVE. Fixtures: pass_via_extension 0 (legal reads + a text-block-quoted forbidden line) / fail_direct_assignment 1 (qualified writes + single-name static import + wildcard static import). WHY: before P2-120 that global had writers spread across 139 files (86 \`*TestSupport.java\` \`useRandomPort\` definitions plus 53 files that assigned it directly; 141 assignment statements in total — census re-measured against f4457530 after a fable5 review found this number stated three different ways), and a global with that many writers admits no reasoning — a file that writes it late, not at all, or with a neighbour's value silently redirects every request that follows, with nothing failing at the point of the mistake. That is one of the two surviving candidates for the P3-144 flake ('a mistargeted port'), and it is concrete rather than theoretical on the machine this was measured on: rest-assured's default port is 8080 and another process there answers \`*:8080\` with a Content-Type-less 404 — byte-identical to the shape P3-144 exhibited, and reproduced verbatim by this lane's differential run. TWO FORBIDDEN SHAPES: (A) an assignment whose target is \`RestAssured.port\`, bare or fully qualified, including the compound forms (+=, -=, *=, /=, %=); \`==\`/\`>=\`/\`<=\`/\`!=\` are comparisons and pass. (B) \`import static io.restassured.RestAssured.port;\`, because Java permits assignment through a single-static-import simple name, so an unqualified \`port = 8080;\` would then be a write that no regex on (A) could ever see. Comments AND string/char literal CONTENTS are blanked before scanning (offsets preserved so line numbers match disk) — load-bearing, not cosmetic, because this tree asserts on the literal \"RestAssured.port=\" in its diagnosability tests and a naive grep would report every one of those as a write. SUBJECT vs OBJECT, NOT AN EXCEPTION: the registered extension assigns the global — that is what makes it the writer — and it is out of the scanned set because it is the rule's SUBJECT. The distinction is DERIVED, never declared: the guard reads the ServiceLoader registration, resolves each FQCN it lists to backend/src/test/java/<path>.java, and excludes exactly those; delete the registration and the extension file is scanned like any other (proven — the same pass fixture flips 0 -> 1 with only its registration removed), and adding a second writer is visible in a diff at the one place that confers the privilege. NO ALLOWLIST — the object set is derived from disk every run and no count is hardcoded, so a new domain is inside the rule the moment its file lands. DELIBERATELY NOT DONE: \`given().port(n)\` is a PER-REQUEST port that touches no global and is not checked (one diagnosability test uses it on purpose); no requirement that a class declare @LocalServerPort at all (a MOCK-environment test legitimately has none, and the extension deliberately leaves the global untouched there — demanding the field would be demanding a lie); no reflective-write detection (the failure mode this exists for is copying a neighbouring line, not defeating a scanner). HISTORY: when this guard first landed its live entry was RED by design, counting the 134 assignments still awaiting deletion — the guard counting what it was written to abolish. Those landed in the same round, so live is GREEN now (498 files scanned, one writer). Recorded because a guard that was born red and is now green should say which it is, and because it must never be weakened to make the suite green. Exit: 0 PASS / 1 a write to the process-global found / 2 usage-environment error. Fixtures: pass_via_extension 0 (a test that reads the global, compares against it, carries the literal \"RestAssured.port=\" in an assertion, mentions the forbidden line in a comment, uses \`given().port(...)\`, and declares a stub aim through AxPort.overrideForStub — every legal shape a substring grep would false-positive on) / fail_direct_assignment 1 (a bare write, a fully-qualified write, a compound write, and the static-import evasion, alongside four reads that must NOT be counted).)"
run_guard "restassured_port_single_writer/live" 0 \
    bash "$SCRIPT_DIR/restassured_port_single_writer_guard.sh"
if [ "$INCLUDE_FIXTURES" -eq 1 ]; then
    run_guard "restassured_port_single_writer/fixture_pass_via_extension" 0 \
        bash "$SCRIPT_DIR/restassured_port_single_writer_guard.sh" --root "$SCRIPT_DIR/fixtures/restassured_port_single_writer/pass_via_extension"
    run_guard "restassured_port_single_writer/fixture_fail_direct_assignment" 1 \
        bash "$SCRIPT_DIR/restassured_port_single_writer_guard.sh" --root "$SCRIPT_DIR/fixtures/restassured_port_single_writer/fail_direct_assignment"
fi
