...
if ! command -v python3 >/dev/null 2>&1 || ! python3 -c 'import yaml' >/dev/null 2>&1; then
    echo "aggregate_boundary_allowlist_guard: BLOCK — cannot verify: python3 + PyYAML required (python3 -m pip install pyyaml)" >&2
    exit 2
fi
...
try:
    ...
    print("aggregate_boundary_allowlist_guard: BLOCK — cannot verify: PyYAML required", file=sys.stderr)
    sys.exit(2)
