...
<!-- @source rules/dogfood-finding-real-bug-must-reference-test-coverage.md -->
...
evidence:
  - source_type: external
    ...
    quoted_at: "2026-06-01"
  - source_type: external
    citation: "IETF draft-ietf-httpapi-idempotency-key-header-07 — The Idempotency-Key HTTP Header Field, §2 The Idempotency-Key HTTP Request Header Field. Establishes that an idempotency key is a unique value the resource uses to recognize retries; a per-(subject,target) marker uses its natural key as that recognizer instead of a client-supplied header."
    url: "https://datatracker.ietf.org/doc/draft-ietf-httpapi-idempotency-key-header/"
...
- **realtime-policy** — applied by: (none)
