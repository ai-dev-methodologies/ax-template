...
class OrderQuantizationComplianceTest {
    ...
    void quantize_roundsUpToLotMultiple_flooredAtMoq() {
        ...
        assertThat(quantize("SKU-A", 10, 1, 1).jsonPath().getLong("orderQuantity")).isEqualTo(10L);

        // required=23, MOQ=1, multiple=10 → ceil(23/10)=3 → 30
        assertThat(quantize("SKU-B", 23, 1, 10).jsonPath().getLong("orderQuantity")).isEqualTo(30L);

        // required=5, MOQ=50, multiple=10 → ceil(5/10)=1*10=10, max(50,10)=50 (MOQ floor dominates)
        assertThat(quantize("SKU-C", 5, 50, 10).jsonPath().getLong("orderQuantity")).isEqualTo(50L);

        // required=0, MOQ=25, multiple=5 → max(25, 0) = 25 (cannot place sub-MOQ order)
        assertThat(quantize("SKU-D", 0, 25, 5).jsonPath().getLong("orderQuantity")).isEqualTo(25L);

        // required=100, MOQ=1, multiple=12 → ceil(100/12)=9*12=108
        assertThat(quantize("SKU-E", 100, 1, 12).jsonPath().getLong("orderQuantity")).isEqualTo(108L);
    }
    ...
    void basis_isRecorded_andReconstructible() {
        String id = quantize("SKU-BASIS", 23, 5, 10).jsonPath().getString("id");
    ...
    void idempotent_sameInputsSameResult() {
        long q1 = quantize("SKU-IDEM", 23, 1, 10).jsonPath().getLong("orderQuantity");
        long q2 = quantize("SKU-IDEM", 23, 1, 10).jsonPath().getLong("orderQuantity");
        long q3 = quantize("SKU-IDEM", 23, 1, 10).jsonPath().getLong("orderQuantity");
        assertThat(q1).isEqualTo(30L);
        assertThat(q2).isEqualTo(q1);
        assertThat(q3).isEqualTo(q1);

        long ov1 = quantize("SKU-IDEM2", 5, 50, 10).jsonPath().getLong("overage");
        long ov2 = quantize("SKU-IDEM2", 5, 50, 10).jsonPath().getLong("overage");
