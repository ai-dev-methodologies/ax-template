...
public class EligibleInvestorController {
    ...
    /**
     * READ-ELIGIBLE-001: list eligible investors for a token.
     * ADMIN-only. Returns 404 for a nonexistent tokenCode.
     * Unbounded list forbidden — uses PageEnvelope (PAGE-OFFSET-001).
     */
    @GetMapping("/api/security-tokens/{tokenCode}/eligible-investors")
    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    public PageEnvelope<GrantDto> list(@PathVariable String tokenCode,
                                       @RequestParam(defaultValue = "0") int page,
                                       @RequestParam(defaultValue = "20") int size) {
        return PageEnvelope.from(
                service.listByTokenCode(tokenCode, page, size),
                g -> new GrantDto(tokenCode, g.getHolderId()));
    }
