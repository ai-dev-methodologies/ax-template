...
## 3. 설치 — 빌드까지 ~15분, 최종 검증(R25)까지 1시간 안팎

> 실측(2026-08-10, 공유 부하 하의 맥): clone+submodule ~10초 · `./gradlew build` **9분57초** ·
> `npm ci` **7초** · `run-all-guards.sh` **10분 이상** · prove 스크립트 2종 각 1초 미만 ·
> `verify-completion.sh`(R25)는 도메인별 테스트를 전부 다시 돌아 **20분 이상**.
> 즉 "깔고 빌드까지"는 15분이면 되지만, **[3-3]의 최종 판정까지 완주하면 1시간 안팎**이다.
> 처음엔 빌드까지만 하고 R25는 첫 변경을 만든 뒤에 돌려도 된다.
