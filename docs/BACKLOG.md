...
| P2 (verification escapes) | 18 | 16 | **~89%** |
| P3 (industry-niche deferrals) | 47 | 6 | ~13% |
| **P0–P3 합계 (수렴 분모)** | **153** | **110** | **~72%** |
...
- [ ] P2-18 practices/ Java-side upstream 스냅숏 본문 87건 미커밋 (P2-1a B3 잔여) — `practices/upstream/_MANIFEST.yaml`(v1.1)은 46개 스냅숏의 id/source/sha/bytes를 기록하나 `{id}.snapshot.md` 본문 파일이 하나도 커밋돼 있지 않아, 해당 upstream_id를 참조하는 87개 룰 quote의 truth를 [74]가 검증 불가(현재 `--allow-missing-snapshot`로 advisory WARN). done-when: 각 source URL에서 스냅숏 본문 fetch(+manifest sha 대조)→`{id}.snapshot.md` 커밋→[74]에서 `--allow-missing-snapshot` 플래그 제거(full strict). 네트워크 작업이라 R25 밖 별도 세션에서 수행. 출처: 2026-07-13 backlog-convergence-wave T6 census.
