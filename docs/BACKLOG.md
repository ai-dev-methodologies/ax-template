...
| Tier | 전체 | closed | 수렴률 |
|---|---|---|---|
| P0 (expiry-bound / live defects) | 29 | 28 | **97%** |
| P1 (generic signature backlog) | 67 | 63 | **94%** |
| P2 (verification escapes) | 23 | 20 | **87%** |
| P3 (industry-niche deferrals) | 50 | 47 | **94%** |
| **P0–P3 합계 (수렴 분모)** | **169** | **158** | **93%** |
...
- [ ] P1-64 money-domain idempotency catch-and-requery breaks on PostgreSQL (aborted-tx 25P02) → HTTP 500 instead of idempotent replay; ALSO a verification escape — catch path structurally untested; H2 MODE=PostgreSQL does statement-level rollback so tests never trip it. Pattern: saveAndFlush/persistAndFlush inside @Transactional, catch(DataIntegrityViolationException), then a SELECT re-query in the SAME poisoned tx. 8 affected files: cashinlieu/CashInLieuService.java:59-63, withholdingsplit/RemittanceService.java:46-49, piecewisedeadband/DeadbandService.java:133-144, uomconversion/UomConversionService.java:146-149, orgscope/OrgScopeService.java:84-90, tokenizedsecurities/EligibleInvestorService.java:53-57, tokenizedsecurities/HolderOwnershipService.java:54-63, tokenizedsecurities/SecurityTokenRegisterService.java:52-61 (latter carries a standing FIXME acknowledging this). Maintainers already documented the bug twice in-tree (duplicatesubmission/DuplicateSubmissionService deliberately omits the requery citing poisoned-tx; the FIXME above). Fix: isolate the discrimination/requery in a REQUIRES_NEW inner tx (per the correct eventingest/EventIngestService precedent) or parse the violated constraint name. Batch-scope across all 8 files. 출처: 2026-07-17 adversarial audit.
- [ ] P1-65 idempotency guard using save() WITHOUT flush is dead code — assigned-id INSERT defers to commit, so catch(DataIntegrityViolationException) never fires and a concurrent race yields HTTP 500 at commit instead of the documented idempotent replay. 2 files: reconciliation/ReconciliationService.java:63-70, derivedstatement/DerivedStatementService.java:66-75. Fix: use saveAndFlush (the codebase idiom, e.g. CashInLieuService) so the unique violation fires inside the try. Distinct mechanism from the sibling catch-and-requery item — that one flushes and dies on the requery; this one never flushes so the catch is unreachable. 출처: 2026-07-17 adversarial audit.
- [ ] P1-66 audit-log export status GET is neither role-gated nor owner-scoped — any authenticated principal (incl ROLE_MEMBER / X-API-Key) with a jobId reads a completed ADMIN/AUDITOR export's downloadUrl + recordCount. Surface auditlog/AuditLogExportController.java:49-60 (GET has no @PreAuthorize; findById(jobId) takes jobId alone, no Authentication → no owner scope), SecurityConfig.java:190 gates /api/audit-logs/** only as authenticated(). The sibling POST enqueue IS @PreAuthorize hasAnyRole(ADMIN,AUDITOR); the spec restricts export reads to ADMIN/AUDITOR. Fix: add @PreAuthorize hasAnyRole(ADMIN,AUDITOR) to the GET and scope findById by caller identity (mirror reportexport/ReportExportController's auth.getName()-scoped getJob, 404 on mismatch). 출처: 2026-07-17 adversarial audit.
- [ ] P1-67 SearchPalette L2 block reads navigator.platform in the render body → deterministic SSR/hydration text mismatch on cross-OS deploy — Surface templates/L2/blocks/search-palette.tsx:135,151-153. `navigator?.platform?.includes('Mac')?'⌘':'Ctrl'` computed in render (no mount-gate, no suppressHydrationWarning): Linux server SSRs 'Ctrl+K', macOS client hydrates '⌘+K' → React hydration-mismatch warning + visible post-hydrate flip for every user whose OS differs from the server. Cosmetic (no data/security impact). Fix: compute the symbol in useEffect+useState gated on mount (default 'Ctrl' during SSR), mirroring the repo's theme-toggle/offline-banner mount-gate pattern. 출처: 2026-07-17 adversarial audit.
...
- [ ] P2-21 R25 hard-guards step never runs trio_integrity_guard/cross_trio_guard against the LIVE repo — the Spec Trio (self-declared primary defense) is fixture-only-gated — Surface practices/evals/run-all-guards.sh:93-105,114-161. These two guards' live run_guard call is stubbed to echo+comment; the real invocations exist ONLY inside the INCLUDE_FIXTURES block, so `run-all-guards.sh --include-fixtures` (the R25 command) validates them against static fixtures, never the live specs/+templates. Repro: corrupt specs/auth-frontend-l0.yaml (auth is full_trio) → the guard run manually exits 1, but verify-completion.sh still PASSES → drifted Spec Trio ships 'done', falsifying the 'gates block non-vacuously' thesis for the primary defense. Fix: add the live-repo run_guard calls for both guards to the always-on section (keep the fixture calls for falsification coverage). 출처: 2026-07-17 adversarial audit.
- [ ] P2-22 31 specs declare domain_mode: full_trio with zero contract AND zero blueprint, and no guard checks the domain_mode-vs-artifact axis — METHODOLOGY.md:504-511 defines full_trio as Backend Trio + Frontend Trio REQUIRED; of 58 full_trio specs, 31 have no templates/L4 dir, no contracts/<name>-*.yaml, no blueprints/<name>-*.yaml (root cause: a blanket retrofit of the field). None of domain_spec_trio_guard.sh / full_trio_spec_backend_or_exempt_guard.sh / spec_item_verification_binding_guard.sh check this. Fix: add a guard asserting every full_trio spec has a matching contract AND blueprint, or reclassify the 31 specs to a mode matching reality. 출처: 2026-07-17 adversarial audit.
- [ ] P2-23 templates/L4 (137 .tsx composition-kit files across 25 domains — the layer fork-receivers actually copy) are structurally excluded from all 14 ax/* ESLint rules AND from tsc; R25 frontend-lint never sees them — Surface practices-react/evals/lint_own_blocks_guard.sh:47-48 (scopes only templates/L2/blocks + L0), frontend/eslint.config.mjs files globs (src/packages/apps only — templates/ is an unreachable sibling), frontend/tsconfig.json include. rich-text-editor-client.tsx already imports @templates/L1 behind a @ts-expect-error admitting tsc can't resolve it. Fix: extend lint_own_blocks_guard.sh to copy+lint templates/L4 (and L1) against the ax/* ruleset and add a tsc pass over the templates tree. 출처: 2026-07-17 adversarial audit.
