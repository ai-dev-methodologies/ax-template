# Frontend Templatization + Full-Stack Methodology Extension — PRD (iter3 revision)

> **Status:** Planner draft for `/ralplan` consensus loop (Step 5, iteration 3).
...
items:
  - id: <DOMAIN-FE-NNN>                   # required, unique
    ...
    static_source_ref: array<string>      # required when backend_operation_id is null.
                                          ...
                                          # Forbidden / ignored when backend_operation_id
                                          # is non-null.
...
- Schema-level mutual exclusion: a route with BOTH null and empty
  ...
  ignores any `static_source_ref` provided.
...
4. **If `domain_mode == frontend_only` (NEW iter3):**
   ...
   4. Apply the same checks to every item in
      `specs/<domain>-frontend-l0.yaml` whose `backend_operation_id` is
      null.
...
`frontend_only` mode (NEW iter3 — 3 fixtures):
