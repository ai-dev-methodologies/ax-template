# Functional Capability Extension PRD — 2026-05-18 (iter3)

> **Status:** Planner iter 3 ULTRA-NARROW revision (ralplan consensus loop, Step 5 → Step 6). Applies the single narrow SP25 mitigation requested by `2026-05-18-functional-extension-critic-codex-iter2.md` (Codex Critic ITERATE verdict — 5/6 CLOSED, 1 PARTIAL: B3 `BaseEntity` in-place). Iter3 patches **only §6.3 SP25 + §6.4 verification-matrix SP25 row + §7.2 BaseEntity ownership row + §SP25 risk card**. Iter2 SP23/SP24/SP26/SP27/SP28/SP29, §1 RALPLAN-DR, §10 honored constraints, §11 out-of-scope, §8 pre-mortem, §12 ADR template all remain unchanged. Ready for Codex Critic iter 3 closure re-review.
...
> **Iter3 delta vs iter2:** see §13 (appended at file tail). 7 surgical edits to SP25 only.
> **Pre-flight disk fact (iter3):** `backend/build.gradle.kts:3` pins `org.springframework.boot 3.2.12` → ships Hibernate ORM 6.4.x → Hibernate `@SoftDelete` declarative annotation IS available on the SP25 classpath. Option A (`@SoftDelete` on `@MappedSuperclass`) is feasible; Option B (per-subclass `@SQLDelete`) is documented as the fallback for any downstream fork that downgrades Hibernate < 6.4.
> **Pre-flight disk fact (iter3):** `templates/backend/BaseEntity.java` is **ABSENT** in the workspace at iter3 start. The 8 existing SP16/17/18/19 entity files (`Notification`, `NotificationPreferences`, `AuditLog`, `StoredFile`, `EmailOutbox`, `EmailTemplate`, `ScheduledTask`, `JobHistory`) were authored without a real common parent. SP25 OWNS the create-or-repair of this file (fills the SP13 gap that iter2 incorrectly assumed closed).
> **Pre-flight disk fact (iter3):** `templates/backend/file-storage/StoredFile.java` confirmed present (was TBD in critic brief — verified).
...
- **Pre-flight Hibernate version check (iter3 narrow fix B3 — Option A vs Option B):**
  - **Command:** `cd backend && ./gradlew dependencies --configuration runtimeClasspath | grep hibernate-core`.
  - **Threshold:** Hibernate ORM ≥ 6.4.0 → Option A (declarative `@SoftDelete` on `@MappedSuperclass`, generates correct per-table SQL automatically). Hibernate ORM < 6.4.0 → Option B (per-entity `@SQLDelete` repeat on each of the 8 subclasses + `@Where(clause="deleted_at IS NULL")` on superclass for read filter).
  - **Current workspace fact:** `backend/build.gradle.kts:3` → `org.springframework.boot 3.2.12` → Hibernate ORM 6.4.x → **Option A is selected as primary**. Option B remains documented as the supported fallback for any downstream fork that downgrades Hibernate < 6.4.
  - **Why this matters (Critic iter2 Defect 2):** Hibernate `@SQLDelete(sql="UPDATE <table> SET deleted_at = ?")` requires a literal table name in the static SQL string. A common `@MappedSuperclass` cannot safely carry one static table-specific SQL string for 8 different tables. Hibernate 6.4's `@SoftDelete` is declarative and generates correct per-table SQL at boot for each concrete subclass — this is the canonical Hibernate-supported soft-delete mechanism for `@MappedSuperclass` since ORM 6.4.
- **Deliverables:**
  - **F3 templates (7 NEW files under `templates/backend/data/` + 1 CREATE of `templates/backend/BaseEntity.java` + 1 NEW Flyway migration template):**
    ...
    - **`templates/backend/BaseEntity.java` (CREATE — fills SP13 gap; iter3 acceptance: if file absent at SP25 start, SP25 creates it):**
      ...
      - Soft-delete (Option A — primary): `@SoftDelete(columnName = "deleted_at")` on the `@MappedSuperclass`. Hibernate 6.4+ generates per-table soft-delete SQL automatically for each concrete subclass.
      - Soft-delete (Option B — fallback if Hibernate < 6.4): `@Where(clause = "deleted_at IS NULL")` on the `@MappedSuperclass` (covers the read filter for all subclasses) **plus** per-subclass `@SQLDelete(sql="UPDATE <concrete_table> SET deleted_at = NOW() WHERE id = ?")` declared individually on each of the 8 subclasses. The decision (A vs B) is recorded in `templates/backend/data/README.md` per the pre-flight check result.
      - `@Column(name = "deleted_at") private Instant deletedAt;` field with read-only getter (writes go through the chosen Hibernate soft-delete mechanism).
      - Existing entity files (`id`, etc.) keep their `@Id` fields locally; BaseEntity provides only the shared audit/version/soft-delete surface.
  ...
  - **Snapshots:** spring-flyway (NEW or extend), spring-data-jpa-auditing (NEW), **hibernate-orm-6.4-soft-delete (NEW — anchors Option A evidence: Hibernate ORM 6.4 `@SoftDelete` reference docs)**.
- **Acceptance criteria:**
...
- **Risks + executable mitigations (Critic Blocker 6):**
  ...
  - **R4 (iter3 NEW — Critic iter2 Defect 2):** Downstream Spring Boot fork downgrades Hibernate < 6.4, breaking Hibernate `@SoftDelete` on `@MappedSuperclass` (Option A primary mechanism).
    - **Owner:** SP25 lead.
    - **Command:** `cd backend && ./gradlew dependencies --configuration runtimeClasspath | grep hibernate-core`.
    - **Threshold:** must be `≥ 6.4.0`; if not, the BaseEntity template MUST flip to Option B (per-subclass `@SQLDelete` + `@Where` on superclass). The `templates/backend/data/README.md` records the decision; the `_check-anchors.sh` validator asserts that BaseEntity carries either `@SoftDelete` (Option A) OR (Option B fallback) `@Where` plus a per-subclass `@SQLDelete` in each of the 8 entity files.
    - **Recovery:** If a downstream fork downgrades Hibernate, the fork receiver re-runs the 8-entity RestAssured smoke (Critic iter2 Defect 4 list) under Option B and updates the README; SP25 template ships both code paths.
...
| SP25 | `/ax-verify-java` | `bash skills/ax-verify-java/scripts/run.sh templates/backend/data/ templates/backend/jobs/ templates/backend/BaseEntity.java templates/backend/data/migrations/` + Flyway migration syntax check (`bash templates/backend/_check-anchors.sh templates/backend/data/migrations/`) + `cd backend && ./gradlew :test --tests BaseEntitySoftDeleteIT` + `cd backend && ./gradlew testAll` | `backend/src/test/java/com/ax/template/baseentity/BaseEntitySoftDeleteIT.java` (RestAssured 8-entity smoke per Critic iter2 Defect 4) + `practices/evals/fixtures/soft_delete_base/fail_soft_delete_no_base/Item.java` + RestAssured `JobDispatchIT.java` + `PageRequestCapIT.java` | All 8 entities (Notification, NotificationPreferences, AuditLog, StoredFile, EmailOutbox, EmailTemplate, ScheduledTask, JobHistory): `DELETE /api/v1/<resource>/{id}` sets `deleted_at` AND default `findAll`/`GET` excludes the soft-deleted row; Flyway migration template is valid PostgreSQL SQL; JPA auditing populates fields; jobs DLQ on failure; paging cap enforced | Pre-SP25: `templates/backend/BaseEntity.java` does NOT exist; soft-delete annotations not applied; archunit fails `BASE_ENTITY_FILE_MISSING` followed by `BASE_ENTITY_NOT_SOFT_DELETE` | `cd backend && ./gradlew :test --tests BaseEntitySoftDeleteIT` (first green requires BaseEntity created + migration template valid + 8-entity smoke pass) | `jpa.audit.fields.populated_ratio` (== 1.0); **`baseentity.soft_delete.applied_count` (entity instances soft-deleted per minute, emitted by `SoftDeleteAspect.java` as Micrometer counter `baseentity.soft_delete.applied_count_total{entity=<simpleClassName>}`)**; `jobs.dlq.row_count` correlates with failures; `paging.max_limit_violations` (== 0); emission: `JobDlqHandler.java` increments Micrometer counter `jobs.dlq.row_count_total{queue=<name>}` |
...
## §9 Open Questions (iter3 — max 1 item per Critic narrow re-review trigger)

> Persisted to `.omc/plans/open-questions.md` per Planner Open Questions protocol.

1. **iter3 residual ambiguity: Option A vs Option B selection moment.** SP25's BaseEntity soft-delete primary is Hibernate 6.4+ `@SoftDelete` (Option A). The pre-flight Hibernate version check is documented to occur at SP25 start (`./gradlew dependencies | grep hibernate-core`), and the workspace currently confirms Option A is feasible. But the PRD does not specify whether the SP25 worker runs the version check ONCE at the top of the SP and commits the choice to `templates/backend/data/README.md`, OR whether the verifier re-checks on every CI run and tolerates a future downgrade by flipping templates automatically. **Iter3 default:** once at SP25 start; choice committed in `templates/backend/data/README.md`; downstream forks that downgrade Hibernate must re-run the 8-entity smoke under Option B manually (no auto-flip in template). Critic to confirm or pick the auto-flip alternative.

(iter2 open questions retained as resolved or still-open advisory — none reopened in iter3:
...
**Ready for Codex Critic iter 3 closure re-review.**

**Re-review scope (narrow, per Critic iter2 trigger):** SP25 only — §6.3 deliverables block (lines for BaseEntity CREATE + Hibernate Option A/B + Flyway migration template), §6.3 acceptance (8-entity RestAssured list including `NotificationPreferences`), §6.3 R4 risk, §6.4 verification matrix SP25 row, §7.2 BaseEntity row + migration row. No broad re-review requested.

**Next step:** Codex Critic iter 3 verifies the 4 specific SP25 defects are closed at binary-implementable resolution. On Critic APPROVE (verdict: APPROVE), commit canonical PRD as TD-2026-05-18-030 ADR, hand off `SP23 → SP25 → SP24 ‖ SP26 → SP27 ‖ SP28 → SP29` to `/team`. SP25 worker: at start of SP, run the pre-flight Hibernate version check and commit the Option A vs Option B decision to `templates/backend/data/README.md` before authoring `templates/backend/BaseEntity.java`.
