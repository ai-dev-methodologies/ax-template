# Frontend Templatization + Full-Stack Methodology Extension — PRD (iter2 revision)

> **Status:** Planner draft for `/ralplan` consensus loop (Step 5, iteration 2).
> **Owner:** Planner agent (oh-my-claudecode:planner).
> **Date:** 2026-05-17.
> **Repo:** `ax-template`.
> **Audience:** Architect (re-review), Critic / Codex (re-review), maintainer (final commit).
> **Format contract:** RALPLAN-DR. Sections A–E of the user brief are LOCKED constraints.
> **Revision provenance:** iter1 REJECTED by Codex Critic. This iter2 applies the
> 7 surgical revisions + hard blockers + Architect's 8 remediations. Diff anchor:
> `docs/superpowers/specs/2026-05-17-frontend-templatization-prd.draft.md` (iter1)
> + `…-critic-codex-iter1.md` + `…-architect-review.md`.
...
backend_spec_ref: <string>                # e.g., "specs/auth-asvs-l1.yaml"
...
items:
  - id: <DOMAIN-FE-NNN>                   # required, unique
    ...
    backend_operation_id: <string|null>   # required key; null only if not API-bound
    backend_spec_ref: <DOMAIN-NNN|null>   # required key; links to backend item
...
```yaml
domain: <string>
backend_contract_ref: <string>            # e.g., "contracts/auth-openapi.yaml"
routes:
  - path: <string>                        # e.g., "/login"
    method: <GET|POST|PUT|DELETE>
    backend_operation_id: <string>        # MUST match an operationId in backend_contract_ref
    params: { ... }
    query: { ... }
    states:
      loading: <slot_ref>
...
Validation: `swagger-cli`-style resolver walks `backend_contract_ref`,
asserts `backend_operation_id` exists in the resolved OpenAPI document.
...
1. Enumerate backend Spec Trio files (`specs/<domain>-*.yaml` where
   `<domain>` ∈ domain_allowlist).
2. For each backend Spec Trio file, read the `frontend_required` field.
3. If `frontend_required: false`, write the file to the allowlist's
   `backend_only` list and skip.
4. If `frontend_required: true`, REQUIRE corresponding files:
   - `specs/<domain>-frontend-l0.yaml`
   - `contracts/<domain>-ui.yaml`
   - `blueprints/<domain>-ui-manifest.yaml`
   FAIL with exit 1 if any is missing.
5. For each route in `contracts/<domain>-ui.yaml`, resolve
   `backend_operation_id` against `backend_contract_ref` OpenAPI document.
   FAIL with exit 1 if the operation ID is not found.
6. For each item in `specs/<domain>-frontend-l0.yaml`, verify
   `backend_spec_ref` resolves to an item id in `backend_spec_ref` (top-level).
   FAIL with exit 1 if unresolved.
7. **Coverage check:** count backend items where `frontend_required: true`
   (per-item or inherited from file-level). For each such backend item,
   confirm at least one frontend item carries the matching
   `backend_spec_ref`. Required ratio: **100%** (every backend item with
   `frontend_required: true` must have a matching frontend item). FAIL
   below 100%.
8. **Zero-scan guard:** if the script walked **zero** files (no domain
   matched the allowlist), FAIL with exit 1 and message `ZERO_SCAN`. This
   closes the iter1 Scenario 1 hard-threshold gap.

**Domain allowlist (configurable, defaults shipped):**

```text
trio_integrity_allowlist:
  full_trio:   [auth, crud, payment, practices]
  backend_only: [ratelimit, security, user]
```

Stored at `practices/evals/trio_integrity_allowlist.yaml`. New domains added
by either (a) updating this file or (b) carrying a `frontend_required: false`
marker in the new backend spec.
...
| SP11 | `/ax-verify-domain practices` | `frontend/playwright.config.ts` | `frontend/tests/practices/all-rules-reachable.spec.ts` | Every rule id from `practices/AGENTS.md` + `practices-react/AGENTS.md` resolves under `/practices/<ruleId>` (200 + title contains id) | Pre-SP11: viewer doesn't exist (404) | `cd frontend && npx playwright test tests/practices/` | `practices.viewer.broken_link.count` (assert == 0) |
