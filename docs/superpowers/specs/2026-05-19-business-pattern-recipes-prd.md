# Business Pattern Recipes PRD — 2026-05-19 (Round 5, ralplan iter 3)

> **Status:** ITER 3 (Planner revision) responding to Codex Critic iter 2 narrow blocker (Korean citation verbatim fidelity). 8 iter-1 blockers remain CLOSED; only the 1 narrow evidence-fidelity blocker is addressed in this iter. Awaits Codex Critic iter 3 narrow re-review.
...
> **Iter 3 delta from iter 2:** Korean citation fidelity narrowed per Critic iter 2. Toss billing URL corrected to `https://docs.tosspayments.com/guides/v2/billing` + verbatim short snippet replaces iter-2 paraphrase. Toss payment-widget citation replaced with verbatim page snippet from `https://docs.tosspayments.com/guides/v2/payment-widget`. Channel Talk citation replaced with verbatim page tagline from `https://channel.io/ko`. Coupang reference downgraded to `provenance_class: internal_design` (developer portal returned HTTP 403 on WebFetch — no verbatim quote could be verified). No other content changes; §10/§11 narrow-updated to reflect new evidence state.
...
- ✅ Evidence-anchored AND verbatim-verified (iter 3): every Korean reference flagged `provenance_class: external` now carries a verbatim short citation re-fetched from the cited URL with `quoted_at`. References whose verbatim text could not be confirmed (Coupang developer portal, HTTP 403) are downgraded to `provenance_class: internal_design` with rationale.
- ✅ Korean enterprise context structurally bound: Toss Payments (자동결제 + 결제위젯) and Channel Talk carry verbatim-verified external citations with URLs; Coupang carries inspirational `internal_design` reference with documented downgrade rationale.
...
| 8 | Korean references structurally true | Iter 2: structurally shaped but verbatim-fidelity FAILED per Critic iter 2 spot check. **Iter 3 closure:** Toss billing — URL corrected to `/guides/v2/billing`, citation replaced with verbatim re-fetched snippet ("자동결제는 정기 배송, 음악 스트리밍과 같은 구독형 서비스에서 사용할 수 있어요."). Toss payment-widget — URL narrowed to `/payment-widget`, citation replaced with verbatim short tagline ("수많은 상점을 분석해서 만든 최적의 결제 UI"). Channel Talk — citation replaced with verbatim landing tagline ("고객 상담의 미래는 AI 입니다"). Coupang — WebFetch returned HTTP 403 against `developers.coupangcorp.com/hc/ko`; downgraded to `provenance_class: internal_design` with explicit rationale (Critic iter 2 fix-protocol option B). | §4 Recipe 1 (Toss billing verbatim); Recipe 2 (Toss payment-widget verbatim + Coupang downgraded); Recipe 3 (Channel Talk verbatim); §10 ("verbatim-verified" + downgrade rationale) |

---

## Final Checklist (Planner iter 3)
...
- [x] Every external Korean citation verbatim-verified OR explicitly downgraded to `internal_design` (iter 3 Critic-blocker-8 narrow closure)
- [ ] Codex Critic iter 3 narrow re-review (next step)
