# Functional Capability Extension PRD — 2026-05-18 (iter4)

> **Status:** Planner iter 4 ULTRA-NARROW revision (ralplan consensus loop, Step 5 → Step 6). Applies the single blocking SP25 mitigation flagged by `2026-05-18-functional-extension-critic-codex-iter3.md` (Codex Critic ITERATE verdict — 3/4 CLOSED, 1 BLOCKING: Hibernate ORM 6.4 `@SoftDelete` is **boolean indicator only**, incompatible with this PRD's `deleted_at TIMESTAMP` contract). Iter4 flips Option A → Option B as the **selected path** for ALL 8 entities (per-subclass `@SQLDelete`) and adds an ArchUnit coverage rule. Iter3 SP23/SP24/SP26/SP27/SP28/SP29, §1 RALPLAN-DR, §10 honored constraints, §11 out-of-scope, §8 pre-mortem, §12 ADR template all remain unchanged. Ready for Codex Critic iter 4 closure re-review.
