...
- NO `applied_recipes: []` empty-list literal anywhere (H2 + M4 fix). Scheduler omits the key per `file-storage` + `practices` precedent.
...
## §4 Deliverable Inventory (2 deliverables, 2 atomic SPs)
...
- NO `applied_recipes: []` literal anywhere (H2 + M4).
- TD-022 DELETED (H2).
...
- **H2 (TD-022 unnecessary; `applied_recipes: []` triggers guard violation):** §8 TD-022 DELETED with strikethrough rationale. §3 Must NOT Have explicit "NO `applied_recipes: []` literal". §4.1 scheduler README "NO `applied_recipes:` key at introduction". §5 Migration step 1 explicit "WITHOUT `applied_recipes:` key". §9 + §12 reaffirm.
...
- **M4 (`applied_recipes: []` triggers guard):** Same closure as H2. Scheduler README has no `applied_recipes:` key.
