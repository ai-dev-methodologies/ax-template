# Functional Capability Extension PRD — 2026-05-18 (iter2)

> **Status:** Planner iter 2 revision (ralplan consensus loop, Step 5 → Step 6). Applies the 6 hard blockers + 1 architect-recommended polish from `2026-05-18-functional-extension-critic-codex-iter1.md` (Codex Critic ITERATE verdict) verbatim, plus the corroborating Architect concerns. DELIBERATE mode auto-enabled (multi-week work, ≥7 SPs, atomic Spec-Trio promotions, cross-stack rules with PII implications, shared infra). Ready for Codex Critic iter 2 narrow re-review.
...
> **Iter2 delta vs iter1 draft:** see §13.
...
- **Inputs:** SP23 done. SP13 common `templates/backend/BaseEntity.java` (the file every existing SP13–SP19 entity extends).
- **Deliverables:**
  - **F3 templates (7 NEW files + 1 in-place edit)** under `templates/backend/data/`:
    ...
    - **`templates/backend/BaseEntity.java` (IN-PLACE EDIT, not a new sibling)** — adds:
      - `@SQLDelete(sql="UPDATE <table> SET deleted_at = ? WHERE id = ?")` class annotation.
      - `@Where(clause = "deleted_at IS NULL")` class annotation.
      - `private Instant deletedAt;` field with getter.
      - Existing fields (`id`, `createdAt`, `updatedAt`, `createdBy`, `updatedBy`, `version`) preserved.
    - **Impact list (in-place edit cascades to):** `Notification.java`, `NotificationPreferences.java` (SP16); `AuditLog.java` (SP17); `StoredFile.java` (SP18); `EmailOutbox.java`, `EmailTemplate.java` (SP19); `ScheduledTask.java`, `JobHistory.java` (SP19). Each inherits soft-delete automatically. **Verification step (acceptance):** RestAssured smoke run on every existing domain repo confirms `GET /api/v1/<resource>/{id}` after `DELETE` returns 404 (row hidden by `@Where`).
  ...
  - **Rule (1):** `soft-delete-only-on-base-entity.md` + failing fixture. `protects_template_id: templates/backend/BaseEntity.java` (the in-place-extended file).
  - **Snapshots:** spring-flyway (NEW or extend), spring-data-jpa-auditing (NEW).
- **Acceptance criteria:**
  - `bash skills/ax-verify-java/scripts/run.sh templates/backend/data/ templates/backend/jobs/ templates/backend/BaseEntity.java` → exit 0.
  - JPA audit RestAssured: insert via authenticated context → `created_by`, `created_at` populated.
  - Soft-delete RestAssured (run against ALL existing domain entities that extend BaseEntity): `DELETE /api/v1/<resource>/{id}` → row's `deleted_at` set; subsequent `GET /api/v1/<resource>/{id}` returns 404. Coverage list: `notifications`, `audit-logs`, `stored-files`, `email-outbox`, `email-templates`, `scheduled-tasks`, `job-history`. Each domain's existing IT must still pass.
...
- **Agent count:** 1 lead + 2 workers (data worker handles in-place BaseEntity edit + 7 new files; jobs worker handles 7 jobs files).
- **Effort:** L (2 d — larger than iter1 estimate due to BaseEntity in-place verification).
- **Rollback boundary:** `git tag sp25-pre-start`. Atomic revert of: `templates/backend/data/**` + `templates/backend/jobs/**` + 1 new rule file + 2 snapshots + **revert in-place edit of `templates/backend/BaseEntity.java` (restore SP13 form)**. If revert needed, the 8 existing entities continue to work because they were forward-compatible with the pre-SP25 BaseEntity.
...
| SP25 | `/ax-verify-java` | `bash skills/ax-verify-java/scripts/run.sh templates/backend/data/ templates/backend/jobs/ templates/backend/BaseEntity.java` + `cd backend && ./gradlew testAll` | `practices/evals/fixtures/soft_delete_base/fail_soft_delete_no_base/Item.java` + RestAssured `JobDispatchIT.java` + `PageRequestCapIT.java` + soft-delete smoke against 8 existing entities | Soft-delete only via in-place-extended BaseEntity; JPA auditing populates fields; jobs DLQ on failure; paging cap enforced; **existing 8-entity ITs still pass after in-place edit** | Pre-SP25: `BaseEntity.java` lacks `@SQLDelete` annotation; archunit fails `BASE_ENTITY_NOT_SOFT_DELETE` | `bash practices/evals/fixtures/soft_delete_base/_run.sh pass` | `jpa.audit.fields.populated_ratio` (== 1.0); `jobs.dlq.row_count` correlates with failures; `paging.max_limit_violations` (== 0); emission: `JobDlqHandler.java` increments Micrometer counter `jobs.dlq.row_count_total{queue=<name>}` |
...
| **`templates/backend/BaseEntity.java`** (in-place soft-delete extension) | **SP25** | SP16/17/18/19 existing entities (Notification, NotificationPreferences, AuditLog, StoredFile, EmailOutbox, EmailTemplate, ScheduledTask, JobHistory) inherit; SP24/SP26/SP27/SP28 read | **Single sole-writer SP**; in-place edit serialized within SP25 lead; no other SP touches this file in this cycle |
...
## §9 Open Questions (iter2 — max 2 items per Critic re-review trigger)

> Persisted to `.omc/plans/open-questions.md` per Planner Open Questions protocol.

1. **Mecab-ko interactive install prompt UX (SP26 R2).** The risk mitigation says `/ax-fork-receiver` install.sh asks the user `[Y/n]` whether to configure mecab-ko. This makes install.sh interactive, which complicates CI/headless install. Should the prompt be: (a) interactive by default (current iter2 wording); (b) interactive only when stdin is a TTY (`[[ -t 0 ]]`), and non-interactive default to `postgres-fts simple tokenizer` otherwise; (c) static blueprint field that fork receiver edits post-install (no prompt at install time)? **Iter2 default:** (b). Critic to confirm or pick (c).
2. **Subcommand telemetry emission path (SP29 acceptance matrix).** `policy_check.false_positive_rate`, `evidence_fetch.refresh.attempts_total`, and `explain.responses.cache_hit_ratio` are named signals in §6.9 row SP29. Iter2 documents them as JSON stdout writes (with optional Micrometer pushgateway push when run in CI). Critic asked in iter1 §H whether emission paths are executable. **Iter2 answer:** yes — the subcommand writes a `${TMPDIR}/ax-verify-<sub>-metrics.json` when invoked with `--metrics` flag, and CI hooks (out of scope for this PRD per §12) can scrape that file. The PRD does not ship CI scrapers. **Open:** is the local-JSON path acceptable, or should iter3 specify a stdin/stdout protocol contract for the metrics shim? **Iter2 default:** local JSON; Critic to confirm sufficient.

(Open questions resolved compared to iter1:
- iter1 Q1 "Tier-1 cap exception" → resolved by Critic Blocker 1: Tier-1 stays at 4; F13/F14/F15 are subcommands.
- iter1 Q2 "Existing L4 Korean string migration scope" → resolved by Critic Blocker 4 Option β: out of scope.
- iter1 Q3 "Search default backend choice" → resolved by SP26 R1: default is `postgres-fts` via static blueprint field; mecab-ko handled by Open Question 1 above.)
...
**Ready for Codex Critic iter 2 narrow re-review.**

**Next step:** Codex Critic iter 2 verifies the 6 hard blockers + 1 architect polish have been closed at binary-implementable resolution. On Critic APPROVE, commit canonical PRD as TD-2026-05-18-030 ADR, hand off `SP23 → SP25 → SP24 ‖ SP26 → SP27 ‖ SP28 → SP29` to `/team`.
