# R10 — api-gateway-relay Recipe PRD — 2026-05-23 (Round 10, ralplan iter 3 APPROVED)

> **Status:** APPROVED (3-iter ralplan consensus; Architect iter 2 APPROVE + Codex Critic iter 3 FINAL APPROVE).
...
- **Per-source-class arithmetic (M1 closure — iter 3 corrected per Codex iter 2 narrow):** Table at L190-L210 contains **21 physical rows including header (L190) + separator (L191)**, **19 data rows** (L192-L210). Source-class breakdown: **verbatim-bearing rows = 7** (6 rows with `Verbatim cite` tag — Kong/AWS/Tyk/Apigee/Toss/NAVER Cloud + 1 `final-verbatim-via-alternate` Cloudflare API Shield bridge); **Downgrade rows = 8** (KakaoCloud × 3 + NHN Cloud × 2 + Naver Cloud × 3 including `/product/applicationService/apiGateway` deep-doc 404 distinct from iter 2 fresh-vendor `/product` catalog-root verbatim hit); **Followed-redirect rows = 3** (Kong 301 + Apigee 301 + NHN 301-302 cascade); **alternate-fetched-as-bridge rows = 1** (Cloudflare `/api-gateway` 404 bridge to API Shield). Sum check: 7 + 8 + 3 + 1 = 19 data rows ✓.
  - Reading guide for the 7 verbatim-bearing rows: Kong (developer.konghq.com 200 OK after 301 from docs.konghq.com); AWS (docs.aws.amazon.com/apigateway/ 200 OK direct); Cloudflare API Shield (developers.cloudflare.com/api-shield/ 200 OK after /api-gateway/ 404 bridge — `final-verbatim-via-alternate`); Tyk (tyk.io/docs/ 200 OK direct); Apigee (docs.cloud.google.com/apigee/docs 200 OK after 301 from cloud.google.com); Toss Payments (docs.tosspayments.com/reference 200 OK adjacent fallback — R9 precedent); NAVER Cloud Platform (www.ncloud.com/product 200 OK fresh-vendor adjacent — iter 2 add per Architect H1).
  - Reading guide for the 8 Downgrade rows: KakaoCloud × 3 (page 404 + homepage shell-only + kakaoi host ECONNREFUSED); NHN Cloud × 2 (302-to-error after 301 chain + meetup posts/250 404); Naver Cloud × 3 (api.ncloud-docs.com 404 + guide.ncloud-docs.com 404 + ncloud.com `/product/applicationService/apiGateway` deep-doc 404 distinct from `/product` catalog-root verbatim hit).
...
**Evidence ledger summary:** **5 English verbatim** (Kong + AWS + Cloudflare API Shield + Tyk + Apigee) + **2 Korean verbatim** (Toss adjacent + NAVER Cloud Platform fresh-vendor iter 2) + **8 documented downgrades** (KakaoCloud × 3 + NHN Cloud × 2 + Naver Cloud × 3 incl. ncloud.com /product/applicationService/apiGateway deep-doc 404) + **3 followed-redirect rows** + **1 alternate-fetched-as-bridge row** (Cloudflare /api-gateway 404 → API Shield). Verbatim-bearing rows = 7 (6 `Verbatim cite` + 1 `final-verbatim-via-alternate`). Source-class totals: 7 + 8 + 3 + 1 = 19 data rows; 21 physical table rows incl. header+separator. (iter 3 corrected per Codex iter 2 narrow M1.)
...
Surgical iter 2 → iter 3 fix per Codex iter 2 narrow verdict (M1 arithmetic miscounts at L218/L220 + §12 evidence summary L396).

- **Codex iter 2 narrow M1 (ledger arithmetic)** — CLOSED.
  - L218: replaced "Verbatim cite rows = 6 … total 19/20" with explicit "21 physical rows; 19 data rows; verbatim-bearing 7 (6 Verbatim cite + 1 final-verbatim-via-alternate); Downgrade 8 (KakaoCloud × 3 + NHN × 2 + Naver × 3); Followed-redirect 3; alternate-bridge 1. Sum 7+8+3+1=19 data ✓."
  - L220: reading guide for Downgrade rows now lists 8 (Naver Cloud × 3 incl. `/product/applicationService/apiGateway` deep-doc 404 made explicit).
  - L396 §12 evidence ledger summary updated to "8 downgrades + 3 followed-redirect + 1 alternate-bridge; 19 data rows; 21 physical".
- **Architect iter 2 INFORMATIONAL (Korean per-INV semantic alignment)** — deferred to R11+ ADR per Architect iter 2 framing. Not iter 3 blocker.
- **All 7 iter 2 closures preserved verbatim** — H1 fresh-vendor + H2 guard audit + M2 TD-028 Korean rotation + M3 disambiguation 4-anchor + M4 Must-Have wording + L Option (a) clean-revert. No semantic change.
- **Ready for Codex iter 3 narrow re-review.** Expected verdict: APPROVE (single arithmetic delta closure).
