...
6. **data-subject-rights-l0** lift (access/erase/rectify/portability against PHI — spec-only, uncomposed). — **DONE (IMW6)**: composed as the backend `dsr/` domain (15 tests: 10 compliance + 5 ViolationProof; testDsr in the verify-completion hard gate). A pre-merge adversarial review (5 read-only skeptic lenses) caught 3 real green-but-buggy defects the 15 passing tests had missed — (a) `DsrRestrictionGate` was consulted on rectify/portability but NOT on access/erase (a restricted subject could still read/erase — fail-OPEN); (b) portability validated format (400) BEFORE the restriction gate (423) — wrong fail-closed order; (c) erasure re-collected the retained manifest on every re-request instead of persisting it (idempotency hole for non-deterministic providers). All 3 fixed + regression-locked with new assertions. The lift() "self-lift without legal-ground" finding was REFUTED as over-reach (the spec permits "the subject's request OR documented legal ground" — self-lift is the subject's request). full_trio spec realized backend-only; frontend trio deferred (next FMW).
