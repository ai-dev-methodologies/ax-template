...
fork ax-template
       ↓
25 L4 domains + 11 active recipes · 233 Java rules · 108 React rules · 15 ESLint rules · 114 hard guards · L0 fork-receiver-kit · L2 rate-limit-banner · AGENTS.md sentinel
       ↓
...
| React/Next.js rule catalog | `practices-react/` — 108 rules / 9 families, citing canonical React 19 / Next.js 16 docs | runs via 3 hard gates (`practices-react/evals/run.sh`) |
...
bash practices/evals/run-all-guards.sh       # 114 hard guards (all PASS expected; the pre-push recency guard runs from .githooks/pre-push, not from here)
