...
- **Spring Boot side** — `@Tag`-based JUnit + RestAssured tests
  (`./gradlew test{Domain}`) + 185-rule Java/Spring catalog
  (`practices/rules/`).
...
fork ax-template
       ↓
25 L4 domains + 11 active recipes · 185 Java rules · 99 React rules · 8 ESLint rules · 73 hard guards · L0 fork-receiver-kit · L2 rate-limit-banner · AGENTS.md sentinel
       ↓
...
| Java/Spring rule catalog | `practices/` — 185 rules / 22+ categories with evidence-anchored frontmatter | runs against backend via `testPractices`; advisory probes via `practices/evals/run.sh` |
...
bash practices/evals/run-all-guards.sh       # 73 hard guards (all PASS expected)
...
./gradlew testPractices         # GREEN — 185 rules
...
### Spring/Java (testPractices — 185 rules / 22+ categories)
...
│   ├── upstream/                   # External doc snapshots
│   ├── eslint-plugin-ax/           # @ax/eslint-plugin-ax — 8 ESLint rules
│   ├── evals/                      # 3 hard gates
│   ├── AGENTS.md                   # AI agent entry point (sha sentinel, auto-regen)
│   ├── SKILL.md                    # subsystem skill
