...
- **Spring Boot side** — `@Tag`-based JUnit + RestAssured tests
  (`./gradlew test{Domain}`) + 178-rule Java/Spring catalog
  (`practices/rules/`).
...
fork ax-template
       ↓
25 L4 domains + 11 active recipes · 178 Java rules · 99 React rules · 8 ESLint rules · 72 hard guards · L0 fork-receiver-kit · L2 rate-limit-banner · AGENTS.md sentinel
       ↓
...
| Java/Spring rule catalog | `practices/` — 178 rules / 22+ categories with evidence-anchored frontmatter | runs against backend via `testPractices`; advisory probes via `practices/evals/run.sh` |
...
bash practices/evals/run-all-guards.sh       # 72 hard guards (all PASS expected)
...
./gradlew testPractices         # GREEN — 178 rules
...
### Spring/Java (testPractices — 178 rules / 22+ categories)
