...
- **Spring Boot side** — `@Tag`-based JUnit + RestAssured tests
  (`./gradlew test{Domain}`) + 128-rule Java/Spring catalog
  (`practices/rules/`).
...
fork ax-template
       ↓
25 L4 domains + 11 active recipes · 128 Java rules · 86 React rules · 7 ESLint rules · 70 hard guards · L0 fork-receiver-kit · L2 rate-limit-banner · AGENTS.md sentinel
       ↓
...
| Java/Spring rule catalog | `practices/` — 128 rules / 22+ categories with evidence-anchored frontmatter | runs against backend via `testPractices`; advisory probes via `practices/evals/run.sh` |
...
./gradlew testPractices         # GREEN — 128 rules
...
### Spring/Java (testPractices — 128 rules / 22+ categories)
