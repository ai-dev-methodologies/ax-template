...
- **Spring Boot side** — `@Tag`-based JUnit + RestAssured tests
  (`./gradlew test{Domain}`) + 183-rule Java/Spring catalog
  (`practices/rules/`).
...
fork ax-template
       ↓
25 L4 domains + 11 active recipes · 183 Java rules · 99 React rules · 8 ESLint rules · 73 hard guards · L0 fork-receiver-kit · L2 rate-limit-banner · AGENTS.md sentinel
       ↓
...
| Java/Spring rule catalog | `practices/` — 183 rules / 22+ categories with evidence-anchored frontmatter | runs against backend via `testPractices`; advisory probes via `practices/evals/run.sh` |
...
./gradlew testPractices         # GREEN — 183 rules
...
### Spring/Java (testPractices — 183 rules / 22+ categories)
