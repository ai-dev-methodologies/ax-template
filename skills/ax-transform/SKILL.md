...
| **practices/ catalog** (188 rules, 22+ categories) | Evidence-anchored Java/Spring rules. AI cannot invent rules at random — every rule has an external URL / quote / RFC / JEP citation. |
...
| **`practices-react/` catalog** (99 rules, 9 families) | Evidence-anchored rules for React 19 / Next.js 16. Mirrors Java catalog discipline. |
...
| **3-tier skill topology** | 3 Tier-1 commands, 8 Tier-2 path-triggered skills, 6 Tier-3 core evidence gates (within the 70-guard suite). Agents navigate via skill invocation graph. |
...
| `practices/AGENTS.md` | The 188 rules in AI-consumable form, with sha256 sentinel |
...
| `practices-react/AGENTS.md` | 86 React rules in AI-consumable form |
...
- [ ] Step 7: Run the guard suite (76 hard guards — 6 core evidence gates + domain/meta guards) — `bash practices/evals/run-all-guards.sh` exits 0
...
### Step 7: Guard suite
Script: `practices/evals/run-all-guards.sh` — chains all 76 hard guards. The 6
