 ...
 * — so they satisfy ax/no-app-local-ui-primitives while keeping the library,
 * editor, tags and search screens consistent in the editorial-luxury voice.
 */
...
export function ErrorState({ message, onRetry }: ErrorStateProps) {
  return (
    <Alert variant="error" className="flex flex-col gap-3">
      <span>{message ?? '문제가 발생했습니다. 잠시 후 다시 시도해 주세요.'}</span>
      {onRetry ? (
        <span>
  ...
  return (
    <Card className="border-dashed">
      <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
        {icon ? (
          <span className="grid h-14 w-14 place-items-center border border-border text-[var(--ax-status-accent-fg)]">
            {icon}
          </span>
        ) : null}
